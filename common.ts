...
import { readableStreamToString, VisierUserManagementError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new VisierUserManagementError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new VisierUserManagementError(e, e.response?.data)
                }
                if (innerError instanceof VisierUserManagementError) {
                    // Got "VisierUserManagementError" from the above try block
                    throw innerError;
                }
